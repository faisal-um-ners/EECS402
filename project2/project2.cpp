...
class ColorClass {
  private:
    ...
    // Private helper method to ensure colors within bounds
    void clipColorValues();
...
int main() {
  ...
  testRowCol.printRowCol();
  cout << ": ";
  if (testImage.getColorAtLocation(testRowCol, 
                                  testColor)) {
    testColor.printComponentValues();
  } else {
    cout << "Invalid Index!";
...
/*
Function: clipColorValues
Description: Ensures the color components of the current
ColorClass object stay within the valid range.
This method modifies the object's internal state directly.
*/
void ColorClass::clipColorValues() {
  if (red < COLOR_MIN_VALUE) {
    red = COLOR_MIN_VALUE;
  } else if (red > COLOR_MAX_VALUE) {
    red = COLOR_MAX_VALUE;
  }

  if (green < COLOR_MIN_VALUE) {
    green = COLOR_MIN_VALUE;
  } else if (green > COLOR_MAX_VALUE) {
    green = COLOR_MAX_VALUE;
  }

  if (blue < COLOR_MIN_VALUE) {
    blue = COLOR_MIN_VALUE;
  } else if (blue > COLOR_MAX_VALUE) {
...
ColorClass::ColorClass()
    : red(COLOR_MAX_VALUE), green(COLOR_MAX_VALUE), blue(COLOR_MAX_VALUE) {}

/*
Constructor initializes color with given values.
*/
ColorClass::ColorClass(const int inRed, const int inGreen, const int inBlue){
  // assign values to color components
  red = inRed;  
  green = inGreen;  
  blue = inBlue; 
  clipColorValues();  // Clip values in place
}
...
bool ColorClass::setTo(const int inRed, const int inGreen, const int inBlue) {
  ...
  int oldRed = red, oldGreen = green, oldBlue = blue;
  clipColorValues(); // Clip values in place
  bool clipStatus = (red != oldRed || green != oldGreen || blue != oldBlue);
  return (clipStatus);
}
...
*/
void RowColumnClass::setRow(const int inRow) { row = inRow; }

/*
Sets only the column value.
*/
void RowColumnClass::setCol(const int inCol) { col = inCol; }

/*
Gets the row value.
*/
int RowColumnClass::getRow() const { return row; }

/*
Gets the column value.
*/
int RowColumnClass::getCol() const { return col; }
...
void ColorImageClass::initializeTo(const ColorClass &inColor) {
  for (int i = START_POINT; i < numRows; i++){
    for (int j = START_POINT; j < numCols; j++){
...
bool ColorImageClass::getColorAtLocation(const RowColumnClass &inRowCol,
  ...
  if (row >= START_POINT && row < numRows && col >= START_POINT &&
      col < numCols) {
